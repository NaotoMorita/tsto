function [F,Isp,Gfmb] = scram_thrust(MCR,TPR,pio_pin,Tmc,Min,Tin,p0,rhoin,Aii,Aio,Ano)
	...
	pejt = p0ej*0.528
	%スロート音速
	cejt= sqrt(2/(kappa+1))*sqrt(kappa*Rej*Tej)
	%スロート速度
	Vejt = cejt;
	%スロート密度
	rhoejt = (2/(kappa+1))^(1/(kappa-1))*(p0ej/Rej/Tej)
	%スロート温度
	Tejt= pejt/Rej/rhoejt
	
	%流出マッハ数
	Mej = 3
	%スロートマッハ数との比によってスロート面積を求める
	ERej = fsolve(@(ER)nozzle_solve(ER,1,Mej,kappa),2)
	Aejt = Aej/ERej;
	
	%流量
	Gej = rhoejt*Vejt*Aejt
	pej = pejt * ((1+(kappa-1)/2*1^2)/(1+(kappa-1)/2*Mej^2))^(kappa/(kappa-1))/1000000
	Tej = Tejt * ((1+(kappa-1)/2*1^2)/(1+(kappa-1)/2*Mej^2))
	cej = cejt *  ((1+(kappa-1)/2*1^2)/(1+(kappa-1)/2*Mej^2))^(1/2);
	Vej = Mej * cej

	
	
	%---------------インテークから、エジェクターロケットともに、流路を二つの面積に合わせたものに変更する
	Mejmi = fsolve(@(Mejmi)nozzle_solve((Aej+Aio)/Aej,Mej,Mejmi,kappa),2)
	pejmi = pej * ((1+(kappa-1)/2*Mej^2)/(1+(kappa-1)/2*Mejmi^2))^(kappa/(kappa-1))
	Tejmi = Tej * ((1+(kappa-1)/2*Mej^2)/(1+(kappa-1)/2*Mejmi^2))
	cejmi = cej *  ((1+(kappa-1)/2*Mej^2)/(1+(kappa-1)/2*Mejmi^2))^(1/2);
	Vejmi = Mejmi * cejmi
	
	Miomi = fsolve(@(Miomi)nozzle_solve((Aej+Aio)/Aio,Mio,Miomi,kappa),2)
	piomi = pio * ((1+(kappa-1)/2*Mej^2)/(1+(kappa-1)/2*Miomi^2))^(kappa/(kappa-1))
	Tiomi = Tej * ((1+(kappa-1)/2*Mej^2)/(1+(kappa-1)/2*Miomi^2))
	ciomi = cej *  ((1+(kappa-1)/2*Mej^2)/(1+(kappa-1)/2*Miomi^2))^(1/2);
	Viomi = Miomi * ciomi
	
	%運動量交換より速度。温度はエンタルピーの加算より
	Gmo = Gej+Gio
	Vmo = (Gej*Vej+Gio*Vio )/Gmo
	Rmo = (Ra*Gio+Rej*Gej)/(Gmo)
	Cpmo = (2*(Gej/18)+1.01*(Gio/28.8))/((Gio/28.8)+(Gej/18))
	Tmo = (Gej*(2*Tejmi)+Gio*(1.01*Tiomi))/(Gmo*Cpmo)
	rhomo = Gmo/(Aej+Aio)/Vmo
	pmo = rhomo*Rmo*Tmo
	...
	%エンタルピーにて水蒸気を考慮すること
	Imbi = Gio*interp1(Air_ent(:,1),Air_ent(:,2),Tmo) + Gej*2*Tmo + Gfmb*interp1(H2_ent(:,1),H2_ent(:,2),Tmo);
	Imbo = Gamb*interp1(Air_ent(:,1),Air_ent(:,2),Tmc) +  Gfmb*interp1(H2_ent(:,1),H2_ent(:,2),Tmc) + Gej*2*Tmc;


	Rmc = (Ra *f+Rf)/(1+f);
	Rmc = (Rmc*Gio+Rej*Gej)/(Gmo)
